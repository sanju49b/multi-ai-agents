            ...
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws token']]) {
                    script {
                        ...
                        aws ecs describe-task-definition --task-definition ${TASK_DEFINITION_FAMILY} --region ${AWS_REGION} --query 'taskDefinition.containerDefinitions[0].environment' > current_env.json
...
[{
  ...
  "essential": true,
  "environment": \$(cat current_env.json),
  "logConfiguration": {
    "logDriver": "awslogs",
    "options": {
...
EOF
                        ...
                        echo "Your Streamlit application should be running on the ECS service."
                        echo "Check the AWS ECS console for the public IP address."
                        
                        # Clean up temporary files
                        rm -f current_env.json container_def.json
                        """
                    }
                }
            }
    ...
    post {
        ...
        success {
            echo 'Pipeline completed successfully! Your application has been deployed to ECS.'
        }
        failure {
